...
- Change primary target from Node to Deno (Node should theoretically still work as long as `fetch()` exists)
- Remove any node-specific code to allow for Deno and browser support
...
- Export more types:
  - BooruCredentials, Derpibooru, XmlBooru, Post, SearchParameters,
    SearchResults, Site
- Correctly mark Hypnohub as not supporting `order:random` (or any random
  sorting)
- Update dependencies
...
- Updated some types to return more useful type unions (like AnySite being a
  union of all site domains)
...
- Fix gelbooru returning wrong creation date
  ([thanks respektive](https://github.com/AtoraSuunva/booru/pull/89))

## 2.5.8

- Fix error when searching rule34.xxx with tags that return no results
  - For some reason, the API literally returns an empty response with HTTP 200
    instead of an empty array (or at least a different status code)

## 2.5.7

- Fix error when splitting by tags when konachan returned an empty string
  ([thanks youliao](https://github.com/AtoraSuunva/booru/pull/88))
- Update dependencies
...
- Fix regression that happened when the previous regression was fixed because
  programming
...
- Handle cases where some Danbooru-based boorus (like realbooru) return
  `"directory": null` in their API response and cause invalid URLs to be
  generated by `booru`
- Slightly prettier `example.js`

## 2.5.3

- Use `encodeURIComponent` instead of `encodeURI` for tags to properly encode
  them
...
- Update API endpoint for r34.xxx, see
  https://github.com/AtoraSuunva/booru/pull/75
- Add `Booru#getSearchUrl`, meant for aiding in debugging
  - You can use this to get the URL that booru would use to search, to inspect
...
- Update dependencies, notably:
  - Typescript 4.4.2
  - Fixes a
    [`path-parse` vulnerability](https://github.com/advisories/GHSA-hj48-42vr-x3v9)
...
- Removed furry.booru.org since they have CloudFlare browser verification
  enabled.
  - As far as I know, there's no (intended) way to bypass this if you're not a
    browser.
...
- Change from `tsc` -> `typescript` for package.json scripts, since `tsc` is
  deprecated.
- Update dependencies
...
- Fix Paheal failing to provide a useful error message.
  - Details: Paheal was returning an HTML error page instead of a JSON response.
    The previous way of scraping an error message off HTML pages failed on the
    page Paheal returned.
- Move from `terser-folder` to a custom `minify.js`, which handles both .js and
  .json minifying
...
- Workaround for JS projects trying to use `new booru(site)` #40
  - Will still "work" for certain projects, but now `booru.forSite(site)` is
    preferred
